...
resource "aws_cloudfront_distribution" "sponsor_app_staging" {
  ...
  aliases = ["sponsorships-staging.kaigionrails.org"]

  viewer_certificate {
    acm_certificate_arn            = aws_acm_certificate.sponsor_app_staging.arn
  ...
  #   prefix          = "cloudfront/sponsor-app-staging/"
  # }

  origin {
    origin_id           = "functionurl"
    domain_name         = replace(replace(aws_lambda_function_url.sponsor_app_web_staging.function_url, "https://", ""), "/", "")
    origin_path         = null
