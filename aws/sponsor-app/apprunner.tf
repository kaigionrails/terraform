...
resource "aws_apprunner_service" "sponsor_app_staging" {
  provider     = aws.usw2
  service_name = "sponsor-app-staging"
  ...
  source_configuration {
    image_repository {
      image_configuration {
        ...
        # See also kaigionrails/sponsor-app/deploy/staging/task_definition.jsonnet
        runtime_environment_secrets = {
          DATABASE_URL              = aws_ssm_parameter.sponsor_app_staging_database_url.arn
          GITHUB_APP_ID             = aws_ssm_parameter.sponsor_app_staging_github_app_id.arn
          GITHUB_CLIENT_ID          = aws_ssm_parameter.sponsor_app_staging_github_client_id.arn
          GITHUB_CLIENT_PRIVATE_KEY = aws_ssm_parameter.sponsor_app_staging_github_client_private_key.arn
          GITHUB_CLIENT_SECRET      = aws_ssm_parameter.sponsor_app_staging_github_client_secret.arn
          GITHUB_REPO               = aws_ssm_parameter.sponsor_app_staging_github_repo.arn
          GOOGLE_CLOUD_CREDENTIALS  = aws_ssm_parameter.sponsor_app_staging_google_cloud_credentials.arn
          MAILGUN_API_KEY           = aws_ssm_parameter.sponsor_app_staging_mailgun_api_key.arn
          MAILGUN_SMTP_LOGIN        = aws_ssm_parameter.sponsor_app_staging_mailgun_smtp_login.arn
          MAILGUN_SMTP_PASSWORD     = aws_ssm_parameter.sponsor_app_staging_mailgun_smtp_password.arn
          MAILGUN_SMTP_PORT         = aws_ssm_parameter.sponsor_app_staging_mailgun_smtp_port.arn
          REDIS_TLS_URL             = aws_ssm_parameter.sponsor_app_staging_redis_tls_url.arn
          REDIS_URL                 = aws_ssm_parameter.sponsor_app_staging_redis_url.arn
          S3_FILES_BUCKET           = aws_ssm_parameter.sponsor_app_staging_s3_files_bucket.arn
          S3_FILES_PREFIX           = aws_ssm_parameter.sponsor_app_staging_s3_files_prefix.arn
          SECRET_KEY_BASE           = aws_ssm_parameter.sponsor_app_staging_secret_key_base.arn
          SENTRY_DSN                = aws_ssm_parameter.sponsor_app_staging_sentry_dsn.arn
          SLACK_WEBHOOK_URL         = aws_ssm_parameter.sponsor_app_staging_slack_webhook_url.arn
          TITO_API_TOKEN            = aws_ssm_parameter.sponsor_app_staging_tito_api_token.arn
        }
...
resource "aws_apprunner_custom_domain_association" "sponsor_app_staging" {
  provider    = aws.usw2
  service_arn = aws_apprunner_service.sponsor_app_staging.arn
